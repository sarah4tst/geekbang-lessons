...
public class UserServiceImpl implements UserService {
    ...
    public boolean register(User user) {
        ...
        if (isUserExistedByName(user.getName())) {
    ...
    public User queryUserByNameAndPassword(String name, String password) {
        TypedQuery<User> query = entityManager.createNamedQuery("User.findByNameAndPassword", User.class);
    ...
    private void validateRegisterRequest(User user) {
        ...
        if (!violations.isEmpty()) {
            throw new ValidationException(violations.stream().map(c -> c.getMessage()).collect(
                Collectors.joining(",")));
        }
    }

    private boolean isUserExistedByName(String name) {
